Describe 'Stop-WslDistribution' -Tag Unit {
    BeforeAll {
    ...
    InModuleScope PowerShellWSL {
        ...
        It 'accepts ValueFromPipelineByPropertyName for Distribution' {
            try {
                [pscustomobject]@{ Distribution = 'Ubuntu' } | Stop-WslDistribution -ErrorAction Stop
                throw 'Expected exception was not thrown.'
            }
            catch {
                $_.FullyQualifiedErrorId | Should -Be 'PowerShellWSLStopInvalidState'
            }
        }
    }
}
